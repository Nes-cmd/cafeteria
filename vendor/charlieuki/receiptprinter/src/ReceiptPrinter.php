...
class ReceiptPrinter
{
    ...
    //my own fun
    public function setOrderAdress($order_adress)
    {
    ...
    public function getPrintableSummary($label, $value, $is_double_width = false) {
        ...
        return '\n'.str_pad($label, $left_cols) . str_pad($formatted_value, $right_cols, ' ', STR_PAD_LEFT);
    }
    ...
    public function printRequest( $with_items = true ) {
        if ($this->printer) {
            // Get request amount
            ...
            $footer = "<h4> POWERED BY WARKAORDER</h4>\n";
            // Init printer settings
            $this->printer->initialize();
            $this->printer->feed();
            $this->printer->setJustification(Printer::JUSTIFY_CENTER);
            ...
            $this->printer->setEmphasis(true);
            $this->printer->textChinese("ORDER (ቦኖ)\n");
            $this->printer->setEmphasis(false);
            $this->printDashedLine();
    ...
    public function printBono($with_items = true) {
        if ($this->printer) {
            // Get total, subtotal, etc
            $subtotal = $this->getPrintableSummary('Subtotal', $this->subtotal);
            //$tax = $this->getPrintableSummary('Tax', $this->tax);
            ...
            //$this->printer->text($tax);
            $this->printer->feed(1);
            $this->printer->text('Order Adress '.$this->order_adress.'\n');
            // Print grand total
            $this->printer->selectPrintMode(Printer::MODE_DOUBLE_WIDTH);
            //$this->printer->text($total);
            $this->printer->feed();
