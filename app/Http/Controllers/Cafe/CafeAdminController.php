...
class CafeAdminController extends Controller
{
    ...
    public function bonoPrinter(Request $request)
    {
        $printer = new Printer();
        $orders = new Order();
        $ids = json_decode($request->ids);
        ...
        $customer_place = CustomerPlace::where('id', $orders->customer_places_id);
        $res = $printer->printBono( auth()->user()->id, $items, $customer_place);
        dd($res);
